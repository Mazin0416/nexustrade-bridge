...
http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    if (reqPath.startsWith('/alpaca/')) {
      ...
      // Data API uses separate host (data.alpaca.markets)
      // /alpaca/data/v1beta1/options/snapshots/SPY → data.alpaca.markets/v1beta1/options/snapshots/SPY
      // /alpaca/account → paper-api.alpaca.markets/v2/account
      const isDataRoute = reqPath.startsWith('/alpaca/data/');
      const hostname    = isDataRoute ? 'data.alpaca.markets' : alpacaHost;
      const alpacaPath  = isDataRoute
        ? reqPath.replace('/alpaca/data', '')   // already has /v1beta1/... or /v2/...
